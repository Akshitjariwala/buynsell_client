...
$(function() {
  for (var i = 0; i < handles.length; i++) {
      option = new Option(handles[i],handles[i]);
      $("#listBox").append(new Option(handles[i],handles[i]));
  }
    $("#listBox").val(selected_state);
    selct_district(selected_state);
});

function selct_district($val)
{
    if($val=='SELECT STATE') {
    var options = '<option value="Select city">Select city</option>';
...
//      var divtag=document.getElementsByClassName("divtag");
//      for(i=0;i<divtag.length;i++)
//      {
//        var state=divtag[i].getAttribute("data-state");
//        if($val==state)
//        {
...
//   if(selected_state==$val)
//   {
//        alert(secondlist.value);
//        $("#secondlist").val(selected_city);
//   }
//   else
//   {
//        $("#secondlist").val("Select city");
//   }
}